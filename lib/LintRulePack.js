...
class LinterRulePack {
  constructor(props = {}) {
    ["directory", "version", "name", "description", "author"].forEach((p) => {
      if (!props[p]) {
        throw new Error(
  ...
  getRulesTable() {
    ...
    data.push(["ID".bold, "Description".bold, "Cleans".bold]);
    ...
    const config = {
      header: {
        alignment: "center",
        content: `${this.name} (v${this.version})`.green.bold,
      },
      columns: {
        0: { width: 24 },
        1: {
  ...
  getCodesTable() {
    ...
    const config = {
      header: {
        alignment: "center",
        content:
          `Rule Pack\n${`${this.name} (v${this.version})`.green.bold}` +
          `\n${this.author}`,
      },
      columns: {
        2: {
