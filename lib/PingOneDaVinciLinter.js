...
class PingOneDaVinciLinter {
  ...
  getRulesToRun(commandLineProps, flowProps, empty) {
    let runRules = [];
    let ignore = [];
    let index;

    // Special case if flow is empty
    if (empty) {
      runRules.push("dv-er-empty-flow-001");
    } else {
    ...
    // Moving the ignore options outside the else so that if it is an empty flow, and for some reason the user wants to
    // ignore the error they have that option
  ...
  lintFlow(props = {}) {
    try {
      ...
      for (const f of this.allFlows) {
        ...
        // Check to see if the flow is empty
        let emptyFlow = false;
        if (Object.keys(f.enabledGraphData.elements).length === 0) {
          emptyFlow = true;
        }

        // Get rules to run and ignore based on either command line options or flow variables
        // Empty flows will return only the empty flow rule
